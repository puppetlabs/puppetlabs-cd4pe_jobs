...
class CD4PEJobRunner < Object
  ...
  def initialize(working_dir:, job_token:, web_ui_endpoint:, job_owner:, job_instance_id:, logger:, windows_job: false, base_64_ca_cert: nil, docker_image: nil, docker_run_args: nil, docker_pull_creds: nil, secrets:)
    ...
    @runtime = get_runtime
    cert_dir = @runtime == 'podman' ? '/etc/containers/certs.d' : '/etc/docker/certs.d'
  ...
  def get_runtime
    ...
    # Try to detect docker second to avoid the possibility that the podman-docker
    ...
    begin
      ...
      # Podman not existing is not an error. It is only a problem
      # if both podman and docker are *both* not found.
      # We rescue to avoid the upstream rescue block from catching
      # and exiting the task early.
    end
    ...
    begin
      ...
      raise "Configured to run job in a container, but did not find docker or podman on the PATH"
    end
  end
